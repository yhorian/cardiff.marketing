...
class ManagerGeneric {
  ...
  constructor(Context) {
    this.componentsFolderPath =
      Context.componentsFolderPath ||
      path_1.default.join(__dirname, '..', 'components')
  ...
  getInjectedScript(clientGeneric) {
    ...
    for (const snippet of [...this.requiredSnippets, ...clientListeners]) {
      ...
      const snippetPath = path_1.default.join(
        __dirname,
        'browser',
        `${snippet}.js`
      )
...
class Manager {
  ...
  async useCache(key, callback, expiry) {
    return await (0, index_1.useCache)(
  ...
  registerWidget(callback) {
    if (
      this.#generic.checkPermissions(
        this.#component,
        constants_1.PERMISSIONS.widget
      )
    ) {
